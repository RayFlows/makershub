...
- 前端、后台和文档站 Dockerfile 使用多阶段构建：`dev` 阶段给 compose 热重载开发使用，
  默认最终阶段只保留构建后的静态文件和 nginx 运行时，供 CI 镜像扫描和后续部署复用。
- 数据库和对象存储使用 Docker volume 保存本地数据。
- 后端容器内的 Python 虚拟环境固定在 `/workspace/.venv/api`，避免开发挂载覆盖依赖环境。
- 后端镜像构建阶段使用根目录 `uv.lock` 和 `uv sync --locked` 安装依赖，运行层不保留 `uv/pip` 工具链，降低生产镜像攻击面。
...
- 后端日志默认同时输出到容器 stdout 和 `logs/` 目录。文件日志按用途分流：
  `app.log` 保存普通运行信息，`error.log` 保存错误，`request.log` 保存请求链路，
  `debug.log` 保存本地调试明细。请求日志会带 `X-Request-ID`，方便把前端报错、
  API 响应和后端日志对齐；日志不会记录请求体。
- 后端默认启用应用层安全边界：安全响应头、请求体大小限制和限流。开发环境
  `HSTS_ENABLED` 默认关闭；生产未显式配置时由 `APP_ENV=production` 自动开启。
- API 容器只透传 `compose.dev.yml` 中列出的环境变量。新增邮件、对象存储或外部服务配置时，
  需要同步补到 `api.environment`，否则 `.env` 中的变量不会进入容器。
