...
modules/points/
  ...
  adjustments/                   # 受控人工调整
    service.py
  rules/                         # 固定规则、临时规则审批、一次性任务模板
    service.py
    repository.py
```

后续积分通知、规则与任务模板关联等能力继续按二级模块新增，不得恢复根目录大文件。
...
```text
modules/resources/
  materials/                     # 已落地：物资分类、物资台账、库存快照
  locations/                     # 待落地：场地、工位、空间结构
  maintenance/                   # 待落地：维护中、下架、损坏状态
```
...
```text
modules/borrowing/
  materials/                     # 已落地：物资申请、审核、取消、归还
  schedules/                     # 待落地：预约时段和冲突检测
  exceptions/                    # 待落地：逾期、损坏、丢失、扣分细则
```
...
```text
modules/workbench/
  models.py                      # 工作台任务模型，后续模型增多后再拆 models/
  constants.py                   # 任务状态、分配方式和可见范围
  tasks/                         # 日常任务、临时任务、审核发分
  schedules/                     # 排班和值班
  duty/                          # 值班申请和值班记录
  cleaning/                      # 打扫卫生、大扫除记录
```

当前已落地 `tasks` 第一阶段闭环：任务发布、悬赏领取、完成提交、发布人审核和按积分规则
发放积分。后续排班、值班和清洁任务继续按二级能力模块补齐。
