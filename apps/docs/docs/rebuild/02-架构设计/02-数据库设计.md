# MakersHub 数据库设计草案

## 目标

本文档记录第一阶段数据库设计草案。当前只定义核心表、关系和约束方向，具体字段长度、索引名称和迁移脚本在实现前继续细化。

数据库采用 MySQL。后端通过 SQLAlchemy 和 Alembic 管理模型与迁移。

## 通用约定

所有核心业务表默认包含：

- `id`：内部主键；
- `created_at`：创建时间；
- `updated_at`：更新时间；
- `deleted_at`：软删除时间，可选；
- `created_by`：创建人，可选；
- `updated_by`：最后修改人，可选；
- `remark`：备注，可选。

重要业务记录不直接物理删除。涉及积分、权限、审批、封禁、撤回、损耗和审计的记录必须保留历史。

## 身份与登录

### `users`

用户主体表，表示系统中的一个人。

核心字段：

- `id`；
- `display_name`；
- `avatar_url`；
- `status`：正常、限制、封禁；
- `last_login_at`。

说明：

- 用户主体不直接等于微信 `openid` 或邮箱。
- 微信身份、邮箱密码账号都挂接到同一个用户主体。
- 普通用户第一版由小程序微信登录创建或找到用户主体，再通过绑定邮箱补齐邮箱密码登录入口。
- 第一个 `999` 是例外，可以由受控运维初始化命令先创建邮箱密码账号对应的用户主体。

### `email_password_accounts`

邮箱密码账号表，用于网页端、后台管理端和第一个 `999` 初始化。

它不是“网页用户表”，也不是“后台用户表”。网页端和后台端都复用同一个
`users` 用户主体；能不能进入后台由权限系统判断。

核心字段：

- `id`；
- `user_id`；
- `email`；
- `password_hash`，可空，邮箱已绑定但首次设置密码前为空；
- `password_set_at`，可空；
- `email_verified_at`；
- `status`。

约束：

- `email` 全局唯一。
- 普通用户的 `email_password_accounts` 应由已绑定邮箱的用户主体生成，不作为网页端自助注册入口。
- 网页端首次邮箱验证码登录只消费已绑定到用户主体的邮箱，验证后强制设置密码。
- `password_hash` 为空时不能进行邮箱密码登录，只能走首次登录设置密码流程。
- 第一个 `999` 初始化可以直接创建邮箱密码账号，后续再绑定微信身份。
- 更换邮箱需要验证旧邮箱和新邮箱，密码保持不变。

### `wechat_accounts`

微信身份表，用于小程序登录。

核心字段：

- `id`；
- `user_id`；
- `openid`；
- `unionid`，可空；
- `session_key_hash`，可选；
- `bound_at`；
- `status`。

约束：

- `openid` 全局唯一。
- 第一版普通用户以微信身份作为进入系统的前置身份，不依赖 `unionid` 才能启动。
- 第一版不允许用户自助解绑微信。

### 微信身份与网页身份如何绑定

绑定点不是一张单独的“微信网页关系表”，而是共同指向同一个 `users.id`：

```text
users.id = 4
├── wechat_accounts.user_id = 4
└── email_password_accounts.user_id = 4
```

也就是说，微信登录和邮箱密码登录只是同一个人的两种登录凭证。小程序首次登录时
先创建 `users` 和 `wechat_accounts`；用户在小程序里绑定邮箱后，再为同一个
`users.id` 创建 `email_password_accounts`。网页端首次登录只接受已经存在的
`email_password_accounts.email`，不会创建一个新的孤立网页用户。

### `auth_sessions`

登录会话表，用于支撑短期 access token 和长期 refresh token。

核心字段：

- `id`；
- `user_id`；
- `refresh_token_hash`，refresh token 哈希值，不保存明文；
- `channel`：登录渠道，例如 `wechat`、`password`；
- `client_type`：客户端类型，例如 `miniapp`、`web`、`admin`；
- `status`：`active`、`revoked` 等；
- `expires_at`：refresh token 过期时间；
- `last_used_at`；
- `revoked_at`；
- `revoke_reason`；
- `user_agent`；
- `ip_address`。

规则：

- access token 是短期 JWT，不落库，但必须携带内部用户 ID 和会话 ID。
- refresh token 是长期随机凭证，只保存哈希值。
- 每次 refresh 都会轮换 refresh token，旧 refresh token 立即失效。
- 退出登录、后台踢下线或风控撤销会话时，应更新 `status`、`revoked_at` 和 `revoke_reason`。
- `/auth/me` 校验 access token 时必须同时检查会话是否仍然有效。

### `email_verification_codes`

邮箱验证码记录表。

核心字段：

- `id`；
- `email`；
- `purpose`：绑定、首次登录、重置密码、更换邮箱；
- `code_hash`；
- `expires_at`；
- `consumed_at`；
- `request_ip`；
- `user_id`，可空。

规则：

- 验证码有效期 5 分钟。
- 同一邮箱 1 小时最多发送 10 次。
- 每次重新请求至少间隔 1 分钟。
- `bind_email` 类验证码由已登录小程序用户发起，用于把邮箱绑定到当前用户主体。
- `first_login` 类验证码用于网页端首次登录，只允许命中已绑定邮箱的用户主体，不创建普通新用户。
- 验证码明文不落库，只保存基于服务端密钥的哈希值。

当前实现状态：

- 已创建 `users`、`email_password_accounts`、`wechat_accounts`、`auth_sessions`、`email_verification_codes`；
- 已允许 `email_password_accounts.password_hash` 在首次设置密码前为空；
- 已创建密码哈希工具；
- 已创建初始化唯一 `999` 的服务；
- 已创建微信用户主体和待设置密码邮箱密码账号的服务层逻辑；
- 已创建微信登录接口、访问令牌签发、refresh token 轮换、退出撤销和当前用户校验接口；
- 已创建已登录用户绑定邮箱的验证码发送、限流、消费和绑定接口；
- 已创建网页端首次邮箱验证码登录、首次设置密码和邮箱密码登录接口；
- 密码重置和更换邮箱接口仍待实现。

## 组织与成员

### `departments`

部门表。

初始部门：

- 宣传部；
- 基管部；
- 项目部；
- 运维部。

核心字段：

- `id`；
- `code`；
- `name`；
- `status`；
- `sort_order`。

### `member_profiles`

成员资料表。

核心字段：

- `id`；
- `user_id`；
- `real_name`；
- `student_id`；
- `phone`；
- `email`；
- `college`；
- `major`；
- `grade`；
- `qq`；
- `bio`。

说明：

- 申请表中能从成员资料带出的字段，不应重复手填。

### `department_memberships`

部门成员关系表。

核心字段：

- `id`；
- `user_id`；
- `department_id`；
- `status`；
- `joined_at`；
- `left_at`。

### `positions`

职务定义表。

初始职务：

- 外部成员、协会会员 `0`；
- 干事；
- 部长；
- 副会长；
- 会长；
- 指导老师；
- 管理员 `998`；
- 超级管理员 `999`。

说明：

- `0` 是最低协会身份，不是后台权限点，也不会自动带来管理能力；
- 微信首次登录创建的普通用户默认拥有 `0`，后续可由后台成员管理改为干事、部长等；
- 后台鉴权仍以权限点和角色为准，不能用职务数字大小直接放行接口。

### `user_positions`

用户职务关系表。

核心字段：

- `id`；
- `user_id`；
- `position_id`；
- `department_id`，可空；
- `scope_type`；
- `scope_id`；
- `granted_by`；
- `granted_at`；
- `revoked_at`。

说明：

- `998/999` 是绑定到用户上的附加系统管理身份，不是独立后台账号。
- 全系统只能有一个有效 `999`。

当前实现状态：

- 已创建 `departments`、`member_profiles` 和 `department_memberships`；
- 首批迁移会初始化宣传部、基管部、项目部、运维部；
- 成员资料中的 `phone` 对应旧小程序接口里的 `phone_num` 语义；
- 已创建 `positions` 和 `user_positions`；
- 首批迁移会初始化干事、部长、副会长、会长、指导老师、`998` 和 `999`；
- `user_positions.department_id` 已补齐到 `departments.id` 的外键约束；
- 后台成员管理、部门调动和职务授予仍待权限系统接入后实现。

## 权限

### `permissions`

权限点表。

核心字段：

- `id`；
- `code`；
- `name`；
- `description`；
- `module`；
- `risk_level`；
- `status`。

说明：

- 接口鉴权检查权限点，不直接使用 `identity_code >= 1` 这类数字比较。
- 当前迁移已创建 `permissions` 表，并写入系统、组织、审计和文件第一批权限点。

### `roles`

角色表。

核心字段：

- `id`；
- `code`；
- `name`；
- `description`；
- `is_system`；
- `status`。

当前预置角色：

- `system_super_admin`：对应唯一 `999` 母账号，包含系统兜底权限和指定/恢复 `998` 权限；
- `system_operator`：对应 `998` 管理员，由唯一 `999` 指定，包含系统兜底权限；
- `organization_manager`：维护成员资料、部门和职务；
- `auditor`：查看审计日志；
- `points_manager`：查看积分账户和积分流水，不包含人工调整积分能力。
- `resource_manager`：资源与借用管理员，代表基管部常态职责打包，包含物资台账维护和物资借用审核/归还权限。

说明：

- 角色是权限点的打包方式，不是接口鉴权的直接条件。
- 第一阶段 `resource_manager` 同时包含 `resources.material.manage` 和 `borrowing.application.review`，符合基管部通常同时负责物资台账和借用审核的业务现实。
- 第一阶段 `borrowing.application.review` 暂时同时承载物资借用审核、正常归还和异常归还；正常归还和异常归还风险不同，前端动作必须拆分，后续再新增异常归还独立权限点。
- 如果后续出现只维护物资台账、只审核借用或按场地分工的情况，应通过新增角色或作用域授权调整，不修改资源域和借用域的状态机。

### `role_permissions`

角色权限关系表。

这是一张链接表，连接 `roles.id` 和 `permissions.id`。它本身不重复保存角色名
或权限名，所以直接看表时只会看到数字外键。人类可读含义需要通过 join 展开。

核心字段：

- `role_id`；
- `permission_id`。

例子：

| role_permissions.role_id | roles.code | role_permissions.permission_id | permissions.code |
| ---: | --- | ---: | --- |
| 3 | `organization_manager` | 3 | `organization.member.manage` |
| 3 | `organization_manager` | 4 | `organization.department.manage` |
| 3 | `organization_manager` | 5 | `organization.position.manage` |

这表示 `organization_manager` 这个角色包含成员资料、部门归属和职务维护权限。
当某个用户通过 `user_role_grants` 获得 `organization_manager` 时，权限服务会
通过 `user_role_grants -> roles -> role_permissions -> permissions` 这条链路
判断他是否拥有目标权限点。

### `user_role_grants`

用户角色授权记录表。

核心字段：

- `id`；
- `user_id`；
- `role_id`；
- `scope_type`；
- `scope_id`；
- `granted_by`；
- `granted_at`；
- `revoked_at`。

说明：

- `revoked_at` 为空表示当前有效；非空表示这条授权已撤销，但历史记录保留用于审计。
- 作用域可以是全局、部门、项目、场地等。
- `998/999` 仍作为系统职务保存在 `user_positions` 中；
- 权限服务会把它们映射到系统兜底权限，不自动映射到组织、积分规则审批、资源等普通业务权限；
- `998/999` 当前拥有 `points.manual.adjust`，只用于积分异常修复和受控人工调整；
- `999` 额外拥有指定或恢复 `998` 的母账号动作权限。

## 文件

### `files`

文件元数据表。

核心字段：

- `id`；
- `owner_user_id`；
- `purpose`；
- `visibility`；
- `storage_provider`；
- `bucket`；
- `object_key`；
- `original_filename`；
- `content_type`；
- `size_bytes`；
- `sha256`；
- `status`；
- `uploaded_at`；
- `deleted_at`。

说明：

- 文件内容仍保存在 MinIO 等对象存储中；
- 业务表只保存 `file_id`，不直接保存对象存储路径；
- `bucket + object_key` 唯一；
- 删除默认先标记元数据状态，物理删除失败时后续通过审计或补偿任务处理。

## 积分与账本

### `point_accounts`

积分账户表。

核心字段：

- `id`；
- `user_id`；
- `balance`；
- `frozen_balance`；
- `status`。

约束：

- 每个用户一个积分账户。
- 不允许透支。
- `balance` 表示总余额，`frozen_balance` 表示冻结余额，可用余额由二者相减得到。

### `point_ledger_entries`

积分流水表。

核心字段：

- `id`；
- `account_id`；
- `user_id`；
- `direction`：收入、支出、冻结、解冻、冻结转扣除、反向修正；
- `amount`；
- `balance_after`；
- `available_balance_after`；
- `frozen_balance_after`；
- `business_type`；
- `business_id`；
- `idempotency_key`；
- `reason`；
- `operator_id`，可空。

约束：

- `idempotency_key` 唯一，防止同一业务事件重复发放积分。
- 流水不删除，撤回通过反向流水修正。
- 流水是积分事实来源，余额缓存只允许 points 服务层维护。

### `point_holds`

积分冻结表。

核心字段：

- `id`；
- `account_id`；
- `user_id`；
- `amount`；
- `business_type`；
- `business_id`；
- `idempotency_key`；
- `status`：冻结中、已解冻、已扣除；
- `reason`；
- `created_by`；
- `released_at`；
- `deducted_at`。

当前实现状态：

- 已创建 `point_accounts`、`point_holds`、`point_ledger_entries`；
- 已为旧用户补齐 0 积分账户，新用户首次读取或发生积分变动时懒创建账户；
- 已完成人工调整、冻结、解冻、冻结转扣除和反向流水修正的服务层写入；
- 已完成后台查询、人工调整和反向流水修正接口。

### `point_rules`

固定积分规则表。

核心字段：

- `id`；
- `code`；
- `name`；
- `amount`；
- `rule_type`；
- `status`；
- `version`；
- `effective_from`；
- `effective_to`；
- `created_by`；
- `updated_by`。

规则：

- 固定规则用于日常任务、值班、打扫卫生等可自动化积分发放；
- `temporary_task_template` 类型由临时积分规则审批生成，任务发布人不能临时改积分；
- 撤回只标记 `status=revoked` 并停止后续使用，不删除规则。

### `temporary_point_rules`

临时积分规则表。

核心字段：

- `id`；
- `name`；
- `task_type`；
- `target_scope`；
- `department_id`，可空；
- `reason`；
- `completion_requirements`；
- `amount_per_completion`；
- `max_participants`；
- `total_points_limit`；
- `effective_from`；
- `effective_to`；
- `approval_status`；
- `applicant_id`；
- `approved_by`；
- `approved_at`；
- `approval_reason`；
- `rejected_by`；
- `rejected_at`；
- `rejection_reason`；
- `generated_point_rule_id`；
- `revoke_status`；
- `revoked_by`；
- `revoked_at`；
- `revoke_reason`；
- `revoke_impact_note`。

规则：

- 特殊非模板任务不能直接发布，必须先申请临时积分规则。
- 临时积分规则审批通过后生成一次性任务模板。
- 撤回默认停止后续使用，不自动追回已发积分。
- 只有规则错误、重复发放、恶意滥用或审批失误等异常场景，才通过反向流水追回积分。

### `temporary_point_rule_events`

临时积分规则生命周期事件表。

核心字段：

- `id`；
- `temporary_rule_id`；
- `event_type`：提交、审批通过、驳回、撤回；
- `actor_id`；
- `reason`；
- `extra`；
- `created_at`。

规则：

- 审计日志记录入口、操作者和请求上下文；
- 事件表记录临时规则自己的状态流转；
- 两者不能互相替代，后续通知和任务取消流程应读取事件表和临时规则状态。

## 工作台

### `workbench_tasks`

工作台任务表。

核心字段：

- `id`；
- `title`；
- `task_type`；
- `assignment_type`：指定任务或悬赏任务；
- `visibility`：部门内、协会内或公开；
- `department_id`，可空；
- `content`；
- `deadline`；
- `status`：待领取、待完成、待审核、已完成、已打回、已取消、规则撤回待处理；
- `publisher_id`；
- `assignee_id`，悬赏任务领取前可空；
- `claimed_at`；
- `point_rule_id`；
- `submission_content`；
- `submitted_at`；
- `reviewed_by`；
- `reviewed_at`；
- `review_comment`；
- `completed_at`；
- `point_ledger_entry_id`。

规则：

- 指定任务发布后直接待完成；
- 悬赏任务发布后待领取，领取后直接待完成；
- 执行人提交完成材料后进入待审核；
- 最终标记任务完成的人必须是任务发布人；
- 任务积分必须引用 `point_rules`，发布人不能临时改积分；
- 审核通过后通过积分域发放积分，并把流水 ID 回写到任务；
- 如果审核时积分规则已撤回，任务进入规则撤回待处理，不自动发分。

## 资源

### `resource_categories`

资源分类表。

核心字段：

- `id`；
- `name`；
- `resource_type`：物资、场地、工位、设备；
- `status`；
- `sort_order`。

### `materials`

物资表。

核心字段：

- `id`；
- `category_id`；
- `name`；
- `description`；
- `location`；
- `cabinet_no`；
- `shelf_no`；
- `total_quantity`；
- `available_quantity`；
- `deposit_points`：单件押金积分；
- `status`：可用、维修中、停用、下架。

### `sites`

场地表。

核心字段：

- `id`；
- `name`；
- `description`；
- `location`；
- `status`。

### `workstations`

工位表。

核心字段：

- `id`；
- `site_id`；
- `name`；
- `type`：流动工位、流动固定工位、管理工位、项目工位；
- `allow_personal_items`；
- `point_rule_type`；
- `status`。

## 借用

### `borrow_applications`

借用申请表。

核心字段：

- `id`；
- `applicant_id`；
- `applicant_name_snapshot`：申请时姓名；
- `applicant_student_id_snapshot`：申请时学号；
- `applicant_phone_snapshot`：申请时手机号；
- `applicant_email_snapshot`：申请时联系邮箱；默认来自登录邮箱，成员资料中填写联系邮箱后优先使用成员资料值；
- `applicant_grade_snapshot`：申请时年级；
- `applicant_major_snapshot`：申请时专业；
- `borrow_type`：物资、场地、工位；
- `usage_type`：个人、项目；
- `project_id`，可空；
- `reason`；
- `expected_return_at`；
- `status`：待审核、已驳回、已通过未归还、已取消、已归还、异常关闭；
- `deposit_points`：申请总押金积分；
- `point_hold_id`，可空；
- `submitted_at`：最新提交/重新提交时间，用于待审核队列排序；
- `cancelled_at`；
- `cancel_reason`。

时间语义：

- `created_at` 表示申请记录首次创建时间，修改申请时不重置；
- `updated_at` 表示记录最后更新时间；
- `submitted_at` 表示最近一次进入待审核队列的提交时间；创建申请和修改申请成功时都要刷新；
- 后台待审核队列按 `submitted_at desc, id desc` 排序，避免重新提交的申请沉在旧创建时间位置。

申请人快照语义：

- 提交申请和修改申请成功时，后端从成员资料写入申请人信息快照；
- 借用申请表单不重复填写姓名、学号、手机号、联系邮箱、年级和专业；
- 联系邮箱默认取登录邮箱，成员可以在资料中修改，修改联系邮箱不等同于更换登录邮箱；
- 联系邮箱必须由后端校验邮箱格式，但不需要验证码确认，因为它不是登录凭证；
- 成员资料缺少姓名、学号、手机号、年级或专业，或联系邮箱和登录邮箱均缺失时，后端拒绝提交或修改借用申请；如果历史数据里已有非法联系邮箱，也应按资料不完整处理；
- 历史详情、后台审核和归还处理展示申请人快照；
- 后台详情页如需展示申请人的当前手机号或联系邮箱，应实时读取 `member_profiles` 和登录邮箱兜底生成只读辅助信息，不在 `borrow_applications` 冗余保存；
- 当前联系方式只用于人工联系，不参与申请事实、审核事实、归还事实和审计快照；
- 后续成员修改个人资料，不应改写既有借用申请中的申请人历史信息；
- `applicant_id` 仍用于权限判断、积分冻结和用户关联。

### `borrow_items`

借用明细表。

核心字段：

- `id`；
- `application_id`；
- `resource_type`；
- `material_id`，可空；
- `quantity`；
- `material_name_snapshot`：申请时物资名称；
- `category_name_snapshot`：申请时物资分类名称，可空；
- `unit_deposit_points_snapshot`：申请时单件押金积分；
- `site_id`，可空；
- `workstation_id`，可空。

快照语义：

- 提交申请和修改申请成功时，后端按当前物资台账写入明细快照；
- 历史详情、审核页和归还页展示物资名称、分类和押金时使用明细快照；
- 审批扣库存、正常归还恢复库存和异常库存修正仍通过 `material_id` 操作当前物资台账；
- 后续物资改名、换分类或调整押金，不应改写既有借用申请的历史展示和本次申请押金记录。

### `borrow_reviews`

借用审核记录表。

核心字段：

- `id`；
- `application_id`；
- `reviewer_id`；
- `decision`：通过、打回；
- `comment`；
- `reviewed_at`。

### `borrow_returns`

归还记录表。

核心字段：

- `id`；
- `application_id`；
- `operator_id`；
- `returned_at`；
- `condition`：正常、损坏、丢失、已损耗；
- `comment`；
- `point_action`。

当前实现状态：

- 已落地 `resource_categories`、`materials`、`borrow_applications`、`borrow_items`、`borrow_reviews`、`borrow_returns`；
- 第一阶段只开放物资借用，`borrow_items.material_id` 指向 `materials.id`；
- 场地、工位借用字段随对应资源表落地后再补；
- 正常归还自动恢复物资可借库存，异常归还全额扣除本次申请押金，不自动恢复库存；
- `borrow_returns.comment` 数据库字段允许为空，以兼容正常归还；服务层要求异常归还必须填写备注；
- `exception_closed` 是第一阶段终态，数据库保留归还记录，不通过删除记录或状态回退撤销异常归还。

## 项目

当前状态：

- 本节是 1.5 阶段数据库设计草案，用于保持项目域边界和后续迁移方向。
- 当前最小端到端验收主线不要求创建项目相关表，也不要求开放项目立项、审核、材料、结项和风控闭环。
- 借用、工位等当前主线如果遇到项目相关字段，应先明确“不开放项目借用/项目工位”，不能为了满足字段引用临时半实现项目域。

### `projects`

项目表。

核心字段：

- `id`；
- `project_no`；
- `name`；
- `type`：个人项目、比赛项目；
- `description`；
- `leader_id`；
- `mentor_name`；
- `mentor_phone`；
- `start_at`；
- `end_at`；
- `status`：草稿、待审核、进行中、已打回、待结项审核、已结项、中断、限制、封禁；
- `is_recruiting`；
- `open_source_agreement_file_id`；
- `open_source_agreement_version`。

### `project_members`

项目成员表。

核心字段：

- `id`；
- `project_id`；
- `user_id`；
- `role`：负责人、成员；
- `joined_at`；
- `left_at`。

### `project_materials`

项目材料表。

核心字段：

- `id`；
- `project_id`；
- `uploader_id`；
- `file_id`；
- `file_name`；
- `file_type`；
- `description`。

### `project_reviews`

项目审核记录表。

核心字段：

- `id`；
- `project_id`；
- `review_type`：立项、结项、进展、中断、恢复；
- `reviewer_id`；
- `decision`；
- `comment`；
- `reviewed_at`。

### `project_risk_records`

项目风控记录表。

核心字段：

- `id`；
- `project_id`；
- `reason`；
- `evidence_file_id`，可空；
- `handler_id`；
- `handled_at`；
- `impact_scope`；
- `release_condition`；
- `appeal_status`。

## 审计与通知

### `audit_logs`

审计日志表。

核心字段：

- `id`；
- `actor_id`；
- `action`；
- `target_type`；
- `target_id`；
- `before_snapshot`；
- `after_snapshot`；
- `extra`；
- `ip_address`；
- `user_agent`；
- `request_id`；
- `reason`；
- `result`；
- `risk_level`；
- `created_at`。

说明：

- 审计日志只追加，不提供通用更新入口；
- 已完成 `audit_logs` 模型、迁移和审计服务；
- 绑定邮箱、首次邮箱登录、设置密码、退出登录和初始化 `999` 已写入审计。

### `notification_messages`

通知消息表。

核心字段：

- `id`；
- `recipient_id`；
- `channel`：站内、邮件、微信订阅消息；
- `template_code`；
- `title`；
- `content`；
- `status`；
- `sent_at`。

## 后续细化

后续实现前还需要继续细化：

- 字段类型、长度和索引名称；
- Alembic 迁移命名规范；
- 软删除和归档策略的统一实现；
- 文件预签名 URL 和统一上传接口设计；
- 审计日志归档、查询过滤和敏感快照脱敏策略。
