...
- 已创建 `departments`、`member_profiles` 和 `department_memberships`；
- 首批迁移会初始化宣传部、基管部、项目部、运维部；
- 成员资料中的 `phone` 对应旧小程序接口里的 `phone_num` 语义；
- 已创建 `positions` 和 `user_positions`；
- 首批迁移会初始化干事、部长、副会长、会长、指导老师、`998` 和 `999`；
- `user_positions.department_id` 已补齐到 `departments.id` 的外键约束；
- 后台成员管理、部门调动和职务授予仍待权限系统接入后实现。
...
- `system_operator`：对应 `998` 管理员，由唯一 `999` 指定，底层能力与 `999` 一致；
...
- 作用域可以是全局、部门、项目、场地等。
- `998/999` 仍作为系统职务保存在 `user_positions` 中；权限服务会把它们映射到权限点；
- `998/999` 底层能力一致，区别是 `998` 必须由唯一 `999` 指定。
