...
- 资源和借用已具备第一阶段主体能力：物资分类、物资台账、库存调整、借用申请、审批扣库存冻结押金、正常归还恢复库存解冻押金、异常归还扣押金。

还没有完成的是“完整业务闭环”：物资借用修改申请 `PATCH`、修改后按 `submitted_at` 重排后台待审核队列、成员网页端资源借用页面、后台真实登录与权限菜单、后台主线审核控制台、场地/工位资源、项目 1.5 阶段、小程序业务页系统适配。当前成员网页端第一阶段只作为端到端验收台和可产品化雏形，不是完整会员门户；当前小程序只验收新版登录和邮箱绑定。
